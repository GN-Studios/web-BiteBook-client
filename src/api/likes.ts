...
export const getLikesByUser = async (userId: string): Promise<Recipe[]> => {
  const res = await api.get<
    Array<{ _id: string; recipeId: RecipeWithLikeInfo | null }>
  >(`/likes/user/${userId}`);

  // map -> may produce nulls when recipeId is not populated; narrow with a type guard
  const maybeRecipes = res.data.map((like) =>
    typeof like.recipeId === "object" ? like.recipeId : null,
  );
  const recipes = maybeRecipes.filter(
    (r): r is RecipeWithLikeInfo => r !== null && r !== undefined,
  );
